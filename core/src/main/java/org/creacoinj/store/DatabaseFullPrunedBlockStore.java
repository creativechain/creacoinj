...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See creacoinj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.creacoinj.core.NetworkParameters}.
     ...
     *
     * <p>The balance {@link org.creacoinj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
