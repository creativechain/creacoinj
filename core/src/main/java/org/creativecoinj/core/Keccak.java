...
public class Keccak {
    ...
    public String getHash(String message, KeccakParams keccakParams) {
        ...
        BigInteger[][] P = padding(message, keccakParams);

        //	    Absorbing phase
        for (BigInteger[] Pi : P) {
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    if ((i + j * 5) < (keccakParams.getR() / w)) {
        ...
        do {
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    if ((5 * i + j) < (keccakParams.getR() / w)) {
                        Z = Z + addZero(reverseToString(S[j][i].toByteArray()), 16).substring(0, 16);
                    }
                }
            }

            doKeccackf(S);
        } while (Z.length() < keccakParams.getOutputLength() * 2);

        return Z.substring(0, keccakParams.getOutputLength() * 2);
    }
    ...
    private BigInteger[][] padding(String message, KeccakParams keccakParams) {
        int size;
        message = message + keccakParams.getD();

        while (((message.length() / 2) * 8 % keccakParams.getR()) != ((keccakParams.getR() - 8))) {
            message = message + "00";
        }

        message = message + "80";
        size = (((message.length() / 2) * 8) / keccakParams.getR());
        ...
        for (int _n = 0; _n < message.length(); _n++) {
            ...
            if ((count * 4 % w) == 0) {
                ...
                String revertString = reverseToString(arrayM[i][j].toByteArray());
    ...
    public String reverseToString(byte[] data) {
        byte[] reverse = reverse(data);
        return Utils.HEX.encode(reverse);
    }
